...
async function upsertUser({ email, username, name, role, password }) {
  return prisma.user.upsert({
    where: { email },
    update: { isVerified: true, password, role, failedLoginAttempts: 0, name, username },
    create: { email, username, password, name, role, isVerified: true }
  });
}

async function upsertCommerce(name, data) {
  return prisma.commerce.upsert({
    where: { name },
    update: data,
    create: { name, ...data }
  });
...
async function main() {
  console.log(`Seeding database on ${getDatabaseHost()}...`);
  const password = await bcrypt.hash('password123', 10);

  const admin = await upsertUser({
    email: 'admin@pandora.com', username: 'admin', name: 'Admin Pandora', role: 'ADMIN', password
  });
  const owner = await upsertUser({
    email: 'owner@pandora.com', username: 'owner', name: 'Lucía Casona', role: 'OWNER', password
  });
  const owner2 = await upsertUser({
    email: 'owner2@pandora.com', username: 'owner2', name: 'Martín Peña', role: 'OWNER', password
  });
  const user = await upsertUser({
    email: 'user@pandora.com', username: 'user', name: 'Ana Socio', role: 'USER', password
  });
  const user2 = await upsertUser({
    email: 'user2@pandora.com', username: 'user2', name: 'Diego Visitante', role: 'USER', password
  });
  ...
  for (const plan of planDefs) {
    await prisma.plan.upsert({
      ...
      update: { name: plan.name, price: plan.price, description: plan.description, benefits: plan.benefits },
      create: plan
    });
  }
  ...
  const catNews = await prisma.articleCategory.upsert({
    where: { name: 'Noticias' }, update: {}, create: { name: 'Noticias', slug: 'noticias' }
  });
  const catInterviews = await prisma.articleCategory.upsert({
    where: { name: 'Entrevistas' }, update: {}, create: { name: 'Entrevistas', slug: 'entrevistas' }
  });
  const catAgenda = await prisma.articleCategory.upsert({
    where: { name: 'Agenda' }, update: {}, create: { name: 'Agenda', slug: 'agenda' }
  });
  ...
  await prisma.coupon.upsert({
    where: { code: 'SALTA20' },
    update: { discountPercent: 20, isActive: true, expiresAt: days(60) },
    create: { code: 'SALTA20', discountPercent: 20, isActive: true, expiresAt: days(60) }
  });
  await prisma.coupon.upsert({
    where: { code: 'ORO10' },
    update: { discountPercent: 10, isActive: true, expiresAt: days(15) },
    create: { code: 'ORO10', discountPercent: 10, isActive: true, expiresAt: days(15) }
  });

  const casona = await upsertCommerce('La Casona de Salta', {
    shortDescription: 'Cocina regional, patio colonial y folklore los viernes.',
    description: 'La Casona de Salta es una casona de principios del siglo XX en el casco histórico. Locro, empanadas al horno de barro, carta de vinos del valle Calchaquí y peña los viernes. Ideal para grupos y visitas que buscan la mesa salteña sin apuro.',
    ...
    attributes: ['wifi', 'ac', 'cards', 'virtual_wallet', 'wine_menu', 'gluten_free', 'pet_friendly', 'groups', 'parking', 'local_produce', 'wheelchair', 'cash_discount'],
    latitude: SALTA.lat,
    longitude: SALTA.lng,
  ...
  const pena = await upsertCommerce('Peña El Cardón', {
    shortDescription: 'Peña con guitarra, vino y empanadas hasta tarde.',
    description: 'El Cardón es una peña clásica a dos cuadras de la plaza. Shows de folklore los jueves y sábados, mesa compartida y carta corta de vinos y cerveza artesanal. Plan Plata: productos destacados y respuesta a comentarios.',
  ...
  const cafe = await upsertCommerce('Café del Cerro', {
    shortDescription: 'Café de especialidad con vista al San Bernardo.',
    description: 'Tostado local, medialunas y WiFi. Plan Free: una sola foto y una categoría, para probar el techo del plan.',
  ...
  const teatro = await upsertCommerce('Teatro Victoria', {
    shortDescription: 'Sala histórica con drama, danza y estrenos locales.',
    description: 'El Victoria combina programación estable y residencias. Platea, palcos y foyer con café. Ficha Oro: video, galería y FAQs de boletería.',
  ...
  const empanadas = await upsertCommerce('Empanadas 9 de Julio', {
    shortDescription: 'Horno de barro a una cuadra de la plaza. Alta pendiente.',
    description: 'Local familiar que acaba de cargar la ficha completa (Plata) y espera validación del admin. Empanadas de corte, humita y rosca. Delivery propio y take away.',
  ...
  const barCerrado = await upsertCommerce('Bar Cerrado', {
    shortDescription: 'Alta rechazada: ficha incompleta y fotos genéricas.',
    description: 'Este local se usó para probar el rechazo. El admin dejó motivo de validación. No debe verse en el listado público.',
    ...
    validatedById: admin.id,
    validatedAt: days(-3),
    validationReason: 'Fotos de stock sin relación con el local y teléfono inexistente. Reenviar con prueba de domicilio.'
  });

  const fogon = await upsertCommerce('Fogón de temporada', {
    shortDescription: 'Asado de campo. Oculto al público (vacaciones).',
    description: 'Parrilla de fin de semana en Chicoana. El owner lo pasó a INACTIVE por temporada. Sirve para ver el panel owner con un local no público.',
  ...
  await prisma.commerce.update({
    where: { id: casona.id },
    data: { categories: { set: [{ id: catGastro.id }, { id: catNight.id }] } }
  });
  await prisma.commerce.update({
    where: { id: pena.id },
    data: { categories: { set: [{ id: catNight.id }, { id: catGastro.id }] } }
  });
  await prisma.commerce.update({
    where: { id: cafe.id },
    data: { categories: { set: [{ id: catGastro.id }] } }
  });
  await prisma.commerce.update({
    where: { id: teatro.id },
    data: { categories: { set: [{ id: catTheater.id }] } }
  });
  await prisma.commerce.update({
    where: { id: empanadas.id },
    data: { categories: { set: [{ id: catGastro.id }] } }
  });
  await prisma.commerce.update({
    where: { id: barCerrado.id },
    data: { categories: { set: [{ id: catNight.id }] } }
  });
  await prisma.commerce.update({
    where: { id: fogon.id },
    data: { categories: { set: [{ id: catGastro.id }] } }
  ...
  const commerceIds = [casona.id, pena.id, cafe.id, teatro.id, empanadas.id, barCerrado.id, fogon.id];
  ...
  await prisma.branch.createMany({
    data: [
      { commerceId: casona.id, name: 'Casa Central', address: casona.address, phone: casona.phone, latitude: casona.latitude, longitude: casona.longitude, isMain: true, openingHours: casona.openingHours },
      { commerceId: casona.id, name: 'Patio Güemes', address: 'Av. Bicentenario 890, Salta', phone: '+54 387 421-8801', latitude: SALTA.lat + 0.008, longitude: SALTA.lng + 0.006, isMain: false, openingHours: 'Vie-Sáb 20:00-01:00' },
  ...
  await prisma.product.createMany({
    data: [
      { commerceId: casona.id, name: 'Locro de olla', description: 'Poroto, maíz, chorizo y verdeo. Plato de sábado.', price: 8900, imageUrl: img('la-casona-food.png') },
      { commerceId: casona.id, name: 'Tabla de quesos y Torrontés', description: 'Quesos de altura y copa de torrontés cafetero.', price: 12500, imageUrl: img('product-wine.png') },
      { commerceId: casona.id, name: 'Empanadas al horno (media docena)', description: 'Corte, humita y roquefort.', price: 5400, imageUrl: img('product-empanada.png') },
      { commerceId: pena.id, name: 'Vino de la casa', description: 'Malbec por copa, ideal para la peña.', price: 3500, imageUrl: img('product-wine.png') },
      { commerceId: pena.id, name: 'Picada criolla', description: 'Salames, queso y aceitunas para compartir.', price: 9800, imageUrl: img('la-casona-food.png') },
      { commerceId: empanadas.id, name: 'Docena mixta', description: 'Corte, pollo y humita. Lista en 15 minutos.', price: 7200, imageUrl: img('product-empanada.png') }
    ]
  });

  await prisma.fAQ.createMany({
    data: [
      { commerceId: casona.id, question: '¿Hay menú vegano?', answer: 'Sí: milanesa de berenjena, locro de calabaza y ensalada de quinoa. Marcá gluten_free al reservar si hace falta.' },
      { commerceId: casona.id, question: '¿Se reserva para grupos?', answer: 'Grupos de 6 o más por WhatsApp al 3874218800. Señal del 30% sábados.' },
      { commerceId: casona.id, question: '¿Hay estacionamiento?', answer: 'Cochera convenida a media cuadra y algunas plazas en Caseros.' },
      { commerceId: teatro.id, question: '¿Cómo retiro entradas?', answer: 'Boletería o link de este evento. Estudiantes con DNI 20% mar/jue.' },
      { commerceId: teatro.id, question: '¿Hay acceso para silla de ruedas?', answer: 'Platea baja y baño adaptado. Avisá a boletería para ubicar.' }
    ]
  });

  await prisma.commerceComment.createMany({
    data: [
      { commerceId: casona.id, userId: user.id, comment: 'El locro del sábado y el patio iluminado. Volvería seguro.', rating: 5, category: 'CALIDAD', isRead: true, commerceReply: 'Gracias Ana, los esperamos el viernes de peña.' },
      { commerceId: casona.id, userId: user2.id, comment: 'Un poco de espera el viernes, pero el servicio lo compensó.', rating: 4, category: 'SERVICIO', isRead: true, commerceReply: 'Abrimos una ronda más de cocina a las 22. Perdón la demora.' },
      { commerceId: casona.id, userName: 'Turista de Tucumán', comment: 'Precios de ciudad, porciones generosas. Ambiente romántico.', rating: 5, category: 'AMBIENTE', isRead: false },
      { commerceId: pena.id, userId: user.id, comment: 'La guitarra a las 23:30 y el vino de la casa. Ideal grupos.', rating: 5, category: 'AMBIENTE', isRead: true, commerceReply: '¡Los jueves hay ronda abierta!' },
      { commerceId: cafe.id, userId: user2.id, comment: 'Buen café, un solo enchufe cerca de la ventana.', rating: 4, category: 'PRECIO', isRead: false }
    ]
  });

  await prisma.commerce.update({
    where: { id: casona.id },
    data: { totalComments: 3, averageRating: 4.7 }
  });

  await prisma.commerceAdvisory.create({
    data: {
      commerceId: casona.id,
      advisorId: admin.id,
      title: 'Subí el menú de invierno al link externo',
      content: 'El tráfico de búsqueda “locro Salta” subió. La ficha Oro ya tiene video; falta destacar el locro en la primera foto.',
      recommendations: '1) Portada con la olla. 2) FAQ de reservas. 3) Evento de peña con ticketUrl.',
      metricsSnapshot: { totalComments: 3, averageRating: 4.7, impressions: 1200, clicks: 84, ctr: '7.00', planLevel: 3 },
      status: 'SENT'
    }
  });
...
`.trim();
  ...
  await prisma.article.upsert({
    where: { slug: 'entrevista-chef-local' },
    update: {
  ...
  await prisma.article.upsert({
    where: { slug: 'agenda-teatro-victoria' },
    update: {
  ...
  await prisma.advertisement.deleteMany({
    where: { title: { in: ['Descubrí Salta de noche', 'Agenda cultural de la semana', 'La Casona destacada'] } }
  });
  ...
  await prisma.submission.deleteMany({
    where: { email: { in: ['ana@example.com', 'diego@example.com', 'lucia@example.com', 'martin@example.com'] } }
  });
  await prisma.submission.createMany({
    data: [
      { type: 'CONTACT', name: 'Ana Socio', email: 'ana@example.com', phone: '3874111000', message: 'Quiero saber cómo cargar un comercio de mi barrio.', status: 'PENDING', userId: user.id },
      { type: 'AD_PROPOSAL', name: 'Agencia Norte', email: 'diego@example.com', message: 'Propuesta de banner 15 días en home.', status: 'PENDING', attachmentUrl: img('banner-home.png') },
      { type: 'MAGAZINE_PROPOSAL', name: 'Lucía Casona', email: 'lucia@example.com', message: 'Nota sobre el locro de invierno.', status: 'PENDING', userId: owner.id },
      { type: 'PLAN_UPGRADE', name: 'Martín Peña', email: 'martin@example.com', message: 'Quiero pasar Café del Cerro de Free a Plata.', status: 'PENDING', userId: owner2.id, attachmentUrl: img('banner-home.png') }
    ]
  });

  await prisma.favorite.deleteMany({ where: { userId: { in: [user.id, user2.id] } } });
  const folkloreEvent = await prisma.event.findFirst({ where: { name: 'Noche de Folklore en La Casona' } });
  const publishedArticle = await prisma.article.findUnique({ where: { slug: 'resurgir-noche-salta' } });
  await prisma.favorite.createMany({
    data: [
      { userId: user.id, resourceType: 'commerce', resourceId: casona.id },
      ...(folkloreEvent ? [{ userId: user.id, resourceType: 'event', resourceId: folkloreEvent.id }] : []),
      { userId: user2.id, resourceType: 'commerce', resourceId: cafe.id },
      ...(publishedArticle ? [{ userId: user2.id, resourceType: 'article', resourceId: publishedArticle.id }] : [])
    ]
  });

  await prisma.notification.deleteMany({ where: { userId: { in: [admin.id, owner.id, owner2.id, user.id] } } });
  await prisma.notification.createMany({
    data: [
      { userId: admin.id, type: 'NEW_COMMERCE_REQUEST', message: 'Empanadas 9 de Julio espera validación.', referenceId: empanadas.id, isRead: false },
      { userId: owner.id, type: 'COMMERCE_VALIDATED', message: 'La Casona de Salta está ACTIVE y destacada.', referenceId: casona.id, isRead: true },
      { userId: owner2.id, type: 'NEW_COMMERCE_REQUEST', message: 'Tu ficha Empanadas 9 de Julio sigue PENDING.', referenceId: empanadas.id, isRead: false }
    ]
  });

  await prisma.newsletterSubscription.upsert({
    where: { email: 'ana@example.com' },
    update: { active: true, userId: user.id },
    create: { email: 'ana@example.com', userId: user.id, active: true }
  });
  await prisma.newsletterSubscription.upsert({
    where: { email: 'visitante@example.com' },
    update: { active: true },
    create: { email: 'visitante@example.com', active: true }
  });

  await prisma.searchQuery.upsert({
    where: { term: 'empanadas' },
    update: { count: 18, lastSearched: new Date() },
    create: { term: 'empanadas', count: 18 }
  });
  await prisma.searchQuery.upsert({
    where: { term: 'peña folklore' },
    update: { count: 9, lastSearched: new Date() },
    create: { term: 'peña folklore', count: 9 }
  });

  await prisma.moderationLog.deleteMany({
    where: { analyzedText: { contains: '[DEMO]' } }
  });
  await prisma.moderationLog.createMany({
    data: [
      { resourceType: 'COMMERCE', resourceId: casona.id, status: 'APPROVED', analyzedText: '[DEMO] La Casona de Salta — ficha oro aprobada por filtro + Groq.', basicFilterResult: { blocked: false }, aiResult: { status: 'APPROVED' } },
      { resourceType: 'COMMERCE', resourceId: empanadas.id, status: 'FLAGGED', analyzedText: '[DEMO] Empanadas 9 de Julio — alta nueva, revisar fotos y teléfono antes de ACTIVAR.', basicFilterResult: { blocked: false }, aiResult: { status: 'FLAGGED', reason: 'Alta reciente con comprobante genérico' } },
      { resourceType: 'COMMENT', resourceId: null, status: 'REJECTED', analyzedText: '[DEMO] comentario con palabra estafa bloqueado por filtro básico.', basicFilterResult: { blocked: true, details: 'estafa' }, aiResult: null }
    ]
  });

  console.log('Seeding completed: comercios, eventos, revista, ads, colas, IA logs y fotos /seed/.');
}
